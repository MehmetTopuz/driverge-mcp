 ...
 */
function companionHeaderEsp32(
  name: string,
  prefix: string,
  includeLine: string,
  bindProtoLine: string,
): GeneratedFile {
...
function halImplI2c(name: string, prefix: string, language: CodegenLanguage): [GeneratedFile, GeneratedFile] {
  const content = [
    ...
    "    (void)addr;",
    `    return i2c_master_transmit_receive(s_${name}_dev, &reg, 1, data, len, ${prefix}_I2C_TIMEOUT_MS);`,
    "}",
    "",
  ...
  const header = companionHeaderEsp32(
    name,
    prefix,
    '#include "driver/i2c_master.h"',
    `esp_err_t ${name}_esp32_bind(i2c_master_bus_handle_t bus, uint32_t scl_speed_hz);`,
...
function halImplCan(name: string, prefix: string, language: CodegenLanguage): [GeneratedFile, GeneratedFile] {
  const content = [
    "/*",
 ...
 */
export function generateEsp32Driver(
  json: DatasheetJson,
